...
source=(
  ...
  0012.patch     # misc additions patch
  0014.patch     # OpenRGB patch
)
validpgpkeys=(
...
prepare() {
  ...
  # delete .git folder to avoid dirty flag
  rm -r .git
...
build() {
  ...
  #make htmldocs # skip building htmldocs speeding up compilation time
}
